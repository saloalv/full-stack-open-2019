...
const Weather = ({city}) => {
    ...
    useEffect(() => {
        Axios.get(`https://api.apixu.com/v1/current.json?key=${APIXU_API_KEY}&q=${city}`)
        .then((response) => setWeather(response.data.current),
         () => console.log("failed to load weather"));
    } , [city]);

    if (APIXU_API_KEY === "") {
        return <div>Apixu API key missing, add it on line 7 of src/components/Weather.js</div>
    }
